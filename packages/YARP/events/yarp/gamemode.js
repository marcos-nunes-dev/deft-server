    ...
    if (user) {
      ...
      if (character) {
        ...
        yarp.labels.forEach((label) => {
          const id_ = player.socialClub;
          let i = label.players.indexOf(id_);
          if (i < 0) {
        ...
        if (yarp.tick % yarp.variables['DayTick'].value == 0) {
          //Pagar Salario
          const jobID = character.jobs[0];
          if (jobID) {
            const jobInfo = yarp.jobs[jobID];
            character.giveMoneyBank(jobInfo.salary);
            character.notifyWithPicture("Salário Disponível", "Fleeca Bank", "Olá "+character.id+", seu salário de R$"+jobInfo.salary+", Já está disponível para saque.", "CHAR_BANK_FLEECA");
          } 
          //Sujeira nos carros
          yarp.vehicles.forEach((veh) => {
            if(veh.dirtyLevel==15){
              //Do Nothing
            }else{
              veh.dirtyLevel++;
              veh.save();              
              const vehMP = yarp.Vehicle.instanceByPlate(veh.plate);
              player.call('dirtyLevel', [player, vehMP, veh.dirtyLevel]);
            }
          });
        }   

        if (yarp.tick % yarp.variables['Fuel Timer'].value == 0) {
          if(player.vehicle){
            player.call('checkIfIsMoving', [player]);
          }
        }   
      }
    }

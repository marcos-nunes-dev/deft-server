...
let deftVersion = "Deft Gamemode Roleplay 1.0";
let fuelScreen = "-";
...
//Display Fuel
mp.events.add('displayFuel', (fuel) => {
  fuelScreen = fuel;
});
...
function displaySpeedometer(mph) {
	if(mp.players.local.isInAnyVehicle(false)) {
  	...
  	mp.game.graphics.drawText(`Speed: ${speed.toFixed(0)} Fuel: ${fuelScreen}`, [0.5, 0.850], {
      font: 7,
...
function displayCredits() {  
  mp.game.graphics.drawText(deftVersion, [0.064, 0.97], { 
    font: 4, 
    color: [1, 236, 144, 255], 
    scale: [0.4, 0.4], 
    outline: false
